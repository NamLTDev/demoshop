...
function billsByMonthAndYear(month, year) {
	...
	$.ajax({
		...
		$.each(result, function(i, bill) {
			$("#tbody").append("<tr><th><a href='' class='link-detail' rowNumber='"+bill.id+"'>Xem chi tiết</a>"+
			...
			"</th><td>"+bill.total+"</td><td>"+bill.payment+"</td><td>"+bill.buyDate+"</td><td>"+bill.user.phone+"</td><td>"+bill.user.name+"</td><td>"+bill.address+"</td><td>"+bill.status.statusName+"</td></tr>");
		});
	});
